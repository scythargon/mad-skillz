...
def my_userAgentForUrl(self, url):
        return "Mozilla/5.0 (X11; Linux x86_64; rv:7.0.1) Gecko/20100101 Firefox/7.0.1"
QWebPage.userAgentForUrl = my_userAgentForUrl
...
class Browser(QObject):
	...
	def check (self):
		i=0
		stop = 5
		while ((not self.js("$('#phone').html()").toString().startsWith(QString('<img src="/items'))) and i<stop):
			print i,': ',self.js("$('#phone').html()").toString()
			time.sleep(1)
			i+=1
		print self.js("$('#phone').html()").toString()
		self.executing = False

	def _request_finished(self):
		print 'loaded'
		self.js("$('.aj').eq(1).click()")
		QTimer().singleShot(0, lambda: self.check())
		#self.js("$('#phone').css('position','fixed')")
		#self.js("$('#phone').css('top','0px')")
		#self.js("$('#phone').css('left','0px')")
		print 'this tread:',self.js("$('#phone').html()").toString()

		#self.executing = False
	def js(self, com):
		return self.browser.currentFrame().evaluateJavaScript(QString(com))
...
def main(argv=None):
	...
	webkit.save_image(url, path, width=1280)
